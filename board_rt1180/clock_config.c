 ...
 *                 source to a stable clock to avoid instruction/data fetch issue during clock updating.
 *END**************************************************************************/
...
__attribute__((weak)) void BOARD_SetFlexspiClock(FLEXSPI_Type *base, uint8_t src, uint32_t divider)
{
    if (FLEXSPI1 == base)
    {
        CCM->CLOCK_ROOT[kCLOCK_Root_Flexspi1].CONTROL =
            CCM_CLOCK_ROOT_CONTROL_MUX(src) | CCM_CLOCK_ROOT_CONTROL_DIV(divider - 1);
        __DSB();
        __ISB();
    ...
    {
        CCM->CLOCK_ROOT[kCLOCK_Root_Flexspi2].CONTROL =
            CCM_CLOCK_ROOT_CONTROL_MUX(src) | CCM_CLOCK_ROOT_CONTROL_DIV(divider - 1);
        __DSB();
        __ISB();
...
void BOARD_BootClockRUN(void)
{
...
#if (__CORTEX_M == 33)
    /* When hyperram(FlexSPI2) is used, CM33 root clock must be higher than 1/4
    ...
    ANADIG_OSC->OSC_24M_CTRL |= ANADIG_OSC_OSC_24M_CTRL_OSC_EN(1) | ANADIG_OSC_OSC_24M_CTRL_BYPASS_EN(0) |
                                ANADIG_OSC_OSC_24M_CTRL_LP_EN(1) | ANADIG_OSC_OSC_24M_CTRL_OSC_24M_GATE(0);
...
#ifndef USE_SDRAM /* SDRAM(SEMC) default use PLL2 PFD0 as root clock from ROM */
    /* Init Sys Pll2. */
...
#ifndef USE_HYPERRAM /* Hyperram(FlexSPI2) default use PLL3 PFD2 as root clock from ROM */
    /* Init Sys Pll3. */
    CLOCK_InitSysPll3();
#endif
...
#ifndef USE_HYPERRAM /* Avoid Configure Clock Root for HyperRAM(FlexSPI2) Interface */
